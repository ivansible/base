...
def main():
    ...
    module = AnsibleModule(
        argument_spec=dict(
            ...
            init_base=dict(type='str', required=False, default='/opt/etc/init.d/'),
        ),
    ...
    init_script = module.params['init_base'] + service
    rc = 0
    out = err = ''
    result = {
        'name': service,
    ...
    if module.params['state'] is not None:
        ...
        if module.params['pattern']:
            ...
            # entware init.d scripts use argument 'check' instead of 'running'
            # (rc, out, err) = module.run_command("%s running" % init_script)
            (rc, out, err) = module.run_command("%s check" % init_script)
            if rc == 0:
                running = True
